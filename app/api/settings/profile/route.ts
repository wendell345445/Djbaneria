...
const schema = z.object({
  workspaceName: z.string().trim().min(2, "Informe o nome do workspace.").max(80, "Nome do workspace muito longo."),
  userName: z.string().trim().min(2, "Informe o nome do usuário.").max(80, "Nome do usuário muito longo."),
});
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!userId) {
      return NextResponse.json({ error: "Usuário do workspace não encontrado." }, { status: 404 });
    }
