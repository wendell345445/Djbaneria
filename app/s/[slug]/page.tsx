...
function getSitePalette(site: PublishedDjSite) {
  const customAccent = typeof site.accentColor === "string" ? normalizeHexColor(site.accentColor) : null;

  const matchedByAccent = customAccent
    ? DJ_SITE_PUBLIC_PALETTES.find((palette) => palette.accent === customAccent)
    : null;
  ...
  const fallbackPalette =
    site.theme === "LUXURY_BLACK"
      ? DJ_SITE_PUBLIC_PALETTES.find((palette) => palette.id === "luxury-gold")!
      : site.theme === "NEON_DARK"
        ? DJ_SITE_PUBLIC_PALETTES.find((palette) => palette.id === "cyberpunk-night")!
        : DJ_SITE_PUBLIC_PALETTES.find((palette) => palette.id === "editorial-light")!;

  return customAccent ? { ...fallbackPalette, accent: customAccent } : fallbackPalette;
}
...
function MixCard({
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/5] overflow-hidden border border-[var(--solid)] bg-[var(--solid)]">
        {imageUrl ? (
          <img
            src={imageUrl}
            alt="Mix cover"
        ...
        <span className="absolute bottom-3 right-3 grid h-11 w-11 place-items-center rounded-full border border-[var(--solid-text)] bg-black/30 text-[var(--solid-text)] backdrop-blur-sm transition group-hover:bg-[var(--paper)] group-hover:text-[var(--ink)]">
  ...
  return (
    ...
    >
      <div className="text-center leading-none">
        <p className="dj-mono text-[10px] font-bold uppercase tracking-[0.06em] text-[var(--ink)] transition-colors group-hover:text-[var(--solid-text)]">
          {eventMonth(event.eventDate)}
        </p>
        <p className="dj-display mt-1 text-[30px] leading-none text-[var(--ink)] transition-colors group-hover:text-[var(--solid-text)]">
          {eventDay(event.eventDate)}
        </p>
      </div>

      <div className="min-w-0">
        <h3 className="line-clamp-1 text-[19px] font-extrabold tracking-[-0.02em] text-[var(--ink)] transition-colors group-hover:text-[var(--solid-text)]">
          {event.title}
        </h3>
        <p className="dj-mono mt-1 line-clamp-1 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-[0.06em] text-[var(--ink-soft)] transition-colors group-hover:text-[var(--solid-text)]/70">
      ...
      <ArrowUpRight
        size={22}
        className="text-[var(--ink-soft)] transition-colors group-hover:text-[var(--solid-text)]"
      />
    </a>
  );
}
  ...
  return (
    ...
    >
      <span className="flex min-w-0 items-center gap-3">
        <span className="grid h-10 w-10 shrink-0 place-items-center text-[var(--ink)] transition-colors group-hover:text-[var(--solid-text)]">
          <BrandIcon brand={brand} />
        </span>
        <span className="line-clamp-1 text-[16px] font-bold tracking-[-0.01em] text-[var(--ink)] transition-colors group-hover:text-[var(--solid-text)]">
          {link.label}
        </span>
      </span>
      <ArrowUpRight
        size={18}
        className="shrink-0 text-[var(--ink-soft)] transition-colors group-hover:text-[var(--solid-text)]"
      />
    </a>
  );
}
...
function TestimonialCard({ item }: { item: PublicTestimonial }) {
  return (
    <div className="flex h-full w-[82vw] max-w-[360px] shrink-0 snap-start flex-col border border-[var(--solid)] p-5 min-[480px]:w-[400px]">
      ...
      <div className="mt-5 flex items-center gap-3 border-t border-[var(--line)] pt-4">
        <div className="h-9 w-9 shrink-0 overflow-hidden border border-[var(--solid)] grayscale">
          {item.avatarUrl ? (
            <img
              src={item.avatarUrl}
              alt={item.name}
...
export default async function PublicDjSitePage({ params }: PageProps) {
  ...
  const sitePalette = getSitePalette(site);
  const accentColor = sitePalette.accent;
  ...
  return (
    <main
      ...
      style={
        {
          "--accent": accentColor,
          "--paper": sitePalette.paper,
          "--ink": sitePalette.ink,
          "--ink-soft": sitePalette.inkSoft,
          "--line": sitePalette.line,
          "--solid": sitePalette.solid,
          "--solid-text": sitePalette.solidText,
        } as CSSProperties
      }
    >
      <style
        dangerouslySetInnerHTML={{
          __html: `
            ...
            .dj-gig:hover { background: var(--solid); }
      ...
      <div className="relative mx-auto min-h-screen w-full max-w-none overflow-hidden pb-24 min-[480px]:max-w-[460px] md:my-6 md:max-w-[460px] md:border md:border-[var(--solid)] lg:max-w-[480px]">
        {/* ── TOP BAR ───────────────────────────────────────── */}
          ...
          <div className="flex min-w-0 items-center justify-center">
            <div className="grid h-9 w-9 shrink-0 place-items-center overflow-hidden rounded-full bg-black/35 p-1 backdrop-blur-sm">
              {site.profileImageUrl ? (
                ...
                <span className="dj-display text-[18px] leading-none text-[var(--solid-text)]">
        ...
        <section className="relative h-[68vh] min-h-[480px] max-h-[640px] overflow-hidden bg-[var(--solid)]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[var(--solid)]/85 via-transparent to-[var(--solid)]/30" />

          <div className="absolute inset-x-0 bottom-0 p-5">
            <p className="dj-mono mb-3 text-[11px] font-bold uppercase tracking-[0.16em] text-[var(--solid-text)]/70">
              {site.headline || "Professional DJ"}
              {site.location ? `  /  ${site.location}` : ""}
            </p>
            <h1 className="dj-display text-[clamp(54px,17vw,86px)] uppercase leading-[0.84] tracking-[-0.02em] text-[var(--solid-text)]">
              {site.artistName}
            </h1>
          </div>
        </section>
          ...
          <div className="mt-7 flex flex-col gap-3">
            {bookingUrl ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="group flex h-[58px] items-center justify-between border border-[var(--solid)] bg-[var(--solid)] px-5 text-[var(--solid-text)] transition hover:bg-[var(--accent)] hover:border-[var(--accent)]"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              {messageUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="dj-mono flex h-[52px] items-center justify-center gap-2 border border-[var(--solid)] text-[12px] font-bold uppercase tracking-[0.12em] text-[var(--ink)] transition hover:bg-[var(--solid)] hover:text-[var(--solid-text)]"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="15"
            ...
            <div className="mt-7 space-y-4">
              {site.bookingEmail ? (
                <a
                  ...
                  className="group flex items-center justify-between gap-4 border-b border-[var(--solid)] pb-3 text-[var(--ink)]"
                >
        ...
        {/* ── STICKY BOOKING BAR ──────────────────────────────── */}
        {bookingUrl ? (
          <div className="sticky bottom-0 z-30 -mx-px border-t border-[var(--solid)] bg-[var(--paper)]/95 p-3 backdrop-blur">
            <a
              href={bookingUrl}
              target="_blank"
              rel="noreferrer"
              className="group flex h-[54px] items-center justify-between border border-[var(--solid)] bg-[var(--solid)] px-5 text-[var(--solid-text)] transition hover:bg-[var(--accent)] hover:border-[var(--accent)]"
            >
              <span className="dj-mono text-[13px] font-bold uppercase tracking-[0.12em]">
