...
const copyByLocale = {
  ...
  en: {
    ...
    previousWindow: "Back",
    nextWindow: "Forward",
    focusClip: "Center clip",
    zoomHint: "Use the zoomed view to pick the clip precisely in long tracks.",
  },
  es: {
    ...
    startAt: "Inicio",
    endAt: "Fin",
    clipDuration: "Duración del video y créditos",
    creditCostHint: "El cargo se basa en la duración elegida al generar.",
...
export function BannerMotionPanel({
  ...
  function getStartFromTimelineClientX(clientX: number, options?: { centerClip?: boolean }) {
    ...
    if (rect.width <= 0 || timelineContentWidth <= 0) return trimStartSeconds;
  ...
  function keepClipVisible(safeStart: number) {
    const scroll = timelineScrollRef.current;
    ...
    if (clipStartPx < visibleStart + padding) {
      scroll.scrollLeft = Math.max(0, clipStartPx - padding);
      return;
    }

    if (clipEndPx > visibleEnd - padding) {
      scroll.scrollLeft = Math.max(0, clipEndPx - scroll.clientWidth + padding);
    }
  }
  ...
  function applyTrimStart(nextStart: number, options?: { preview?: boolean; keepVisible?: boolean }) {
    ...
    if (options?.keepVisible !== false) {
      window.requestAnimationFrame(() => keepClipVisible(safeStart));
    }
  ...
  function focusTimelineOnClip() {
    keepClipVisible(trimStartSeconds);
  }
  ...
  return (
    <section id="motion" className="mt-5 scroll-mt-6 rounded-[24px] border border-cyan-300/15 bg-white/[0.045] p-5 shadow-[0_24px_80px_rgba(0,0,0,0.24)]">
      ...
      {activeMotion?.status === "COMPLETED" && activeMotion.outputVideoUrl ? (
        <div className="mt-6 rounded-[24px] border border-cyan-200/20 bg-cyan-200/[0.06] p-4 shadow-[0_24px_80px_rgba(34,211,238,0.10)]">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
            ...
            <a
              ...
              download
              className="inline-flex min-h-11 items-center justify-center rounded-xl bg-white px-4 py-3 text-sm font-black text-slate-950 transition hover:scale-[1.01]"
            >
              {copy.download}
            </a>
          </div>
          ...
          <video
            ...
            controls
            playsInline
            className="mt-4 aspect-[2/3] w-full rounded-2xl bg-black object-cover shadow-[0_22px_70px_rgba(0,0,0,0.32)]"
          />
        </div>
      ) : null}
            ...
            {motions.map((motion) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <span className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs font-bold text-white/70">
                        ...
                        {motion.status === "PENDING" && motion.queuePosition
                          ? ` · ${copy.queuePosition}: ${motion.queuePosition}`
                          : ` · ${progress}%`}
                      </span>
                    </div>
                  ...
                  {isCompleted && motion.outputVideoUrl ? (
                    <div className="mt-3 flex flex-col gap-2 sm:flex-row">
                      <button
                        type="button"
                        onClick={() => setActiveMotionId(motion.id)}
                        className="inline-flex min-h-10 flex-1 items-center justify-center rounded-xl border border-cyan-200/20 bg-cyan-200/[0.08] px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-cyan-100 transition hover:bg-cyan-200/[0.14]"
                      >
                        {copy.preview}
                      </button>
                      <a
                        href={motion.outputVideoUrl}
                        download
                        className="inline-flex min-h-10 flex-1 items-center justify-center rounded-xl bg-white px-4 py-2 text-xs font-black uppercase tracking-[0.12em] text-slate-950 transition hover:scale-[1.01]"
                      >
                        {copy.download}
                      </a>
                    </div>
                  ...
                  >
                    <div className="absolute inset-0 opacity-60">
                      {Array.from({ length: Math.max(120, Math.min(420, Math.ceil(audioDuration || 120))) }).map((_, index, bars) => {
                        ...
                        const height =
                          ...
                          Math.abs(Math.cos(virtualSecond * 1.27)) * 34;
                    ...
                    <div
                      ...
                      style={{
                        ...
                        width: `${Math.max(24, (clipWidthPercent / 100) * timelineContentWidth)}px`,
                        touchAction: "none",
                      }}
                    >
